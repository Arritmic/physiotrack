"""ZipDepth network architecture (vendored, inference-only).

A lightweight, fully self-contained monocular depth estimator: a
reparameterizable RepVGG-style encoder, an FPN decoder, and a convex
upsampling head. Depends only on ``torch`` — no external backbones. Training,
loss, and evaluation code from the upstream ZipDepth project is intentionally
not vendored here; only the modules required to run inference are kept.

The two published checkpoints (``base`` and ``base_npu``) share the same
``variant='base'``/``global_mode='balanced'`` encoder and decoder weights and
differ only in the upsampling head (``upsample_unfold=True`` for the GPU/server
``base`` checkpoint, ``False`` for the NPU/CPU/mobile-friendly one).
"""
